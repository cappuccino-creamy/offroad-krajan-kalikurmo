...
<head>
    ...
    <title>Daftar Akun - Kalikurmo Offroad</title>
...
<body class="bg-gray-100 flex flex-col min-h-screen font-sans text-gray-800">
    ...
    <main class="flex-grow flex items-center justify-center px-4 py-12">
        <div class="bg-white p-8 rounded-3xl shadow-xl w-full max-w-md border-t-8 border-primary">
            <h2 class="text-2xl font-bold text-center text-gray-800 mb-2">Buat Akun Baru</h2>
            <p class="text-center text-gray-500 text-sm mb-8">Bergabunglah untuk reservasi paket wisata.</p>
            ...
            <form action="{{ url('/register') }}" method="POST" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="nama" value="{{ old('nama') }}" required class="w-full bg-gray-50 border rounded-xl px-4 py-3 outline-none focus:border-primary transition" placeholder="Masukkan nama anda">
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">Email</label>
                    <input type="email" name="email" value="{{ old('email') }}" required class="w-full bg-gray-50 border rounded-xl px-4 py-3 outline-none focus:border-primary transition" placeholder="email@contoh.com">
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">Password</label>
                    <div class="relative">
                        <input type="password" name="password" id="reg-password" minlength="6" required class="w-full bg-gray-50 border rounded-xl px-4 py-3 outline-none focus:border-primary transition pr-12" placeholder="Minimal 6 karakter">
                        <button type="button" onclick="togglePassword('reg-password', 'eye-reg')" class="absolute inset-y-0 right-0 flex items-center pr-4 text-gray-400 hover:text-primary transition focus:outline-none">
                            <i id="eye-reg" class="fa-regular fa-eye"></i>
                        </button>
                    </div>
                </div>
            ...
            <p class="text-center mt-6 text-sm text-gray-600">
                Sudah punya akun? <a href="{{ url('/login') }}" class="text-primary font-bold hover:underline">Masuk di sini</a>
            </p>
        </div>
    </main>

    <footer class="bg-secondary text-white py-6 text-center text-sm mt-auto border-t-4 border-accent">
