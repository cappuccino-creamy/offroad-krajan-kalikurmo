...
if (!is_dir($layouts_dir)) mkdir($layouts_dir);

$sidebar_content = <<<'EOD'
@php
    $role = session('role');
    $is_super = $role === 'admin';
    $bg_sidebar = $is_super ? 'bg-primary' : 'bg-blue-800';
@endphp
...
file_put_contents($layouts_dir . '/sidebar.blade.php', $sidebar_content);

$files = [
    'admin-dashboard.blade.php', 'crud-users.blade.php', 'crud-paket.blade.php',
    'crud-reservasi.blade.php', 'crud-galeri.blade.php', 'crud-slider.blade.php',
    'pengaturan-admin.blade.php', 'tambah-paket.blade.php', 'edit-paket.blade.php'
];
...
foreach ($files as $file) {
    ...
    if (file_exists($path)) {
        ...
        // Replace <aside id="sidebar"> ... </aside> with @include('layouts.sidebar')
        $content = preg_replace('/<aside id="sidebar".*?<\/aside>/s', "@include('layouts.sidebar')", $content);
